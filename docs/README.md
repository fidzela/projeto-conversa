...
| [11 — O SEGUNDO CORAÇÃO: JetFormBuilder e o fluxo de envio](11-segundo-coracao-jetformbuilder-fluxo-de-envio.md) | **A interface de envio** (o composer/JFB, onde o usuário interage). O pipeline de envio do JFB, o que o plugin já "passou por cima", os **dois modos de validação** e como ativá-los sem quebrar, por que **Limit Form Responses** não serve ao chat, quais camadas de **segurança/captcha** ligar, o roteiro de ativação segura e a config do Media Field. **Leia antes de mexer na validação/segurança do form.** |
| [12 — Segurança: modelo de ameaças e hardenings (1.2.0)](12-seguranca.md) | A auditoria de segurança: o INSERT nativo do CCT **não autoriza nada** (cross-conversation, spoof de `from_user`, IDOR de mídia, flood, CSRF) e o **guard** que fecha isso via hooks nativos (JFB `before-send` + CCT `item-to-update`). Nonce vinculado à conversa, sanitização do `widget_settings`, anti-flood de envio (`rate_send`) e o que continua sendo configuração de autoria. **Leia antes de mexer em permissões/endpoints.** |
